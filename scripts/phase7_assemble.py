#!/usr/bin/env python3
"""Phase 7 — assemble vertical webtoon PNG + multi-page PDF per story."""
...
def main() -> None:
    ...
    observer.upsert(
        "phase7",
        title="Phase 7 — Episode assembler",
        description="Vertical webtoon PNG + multi-page PDF per story",
        phase="phase7",
        sort_order=70,
        status="in_progress",
        progress=0.05,
    ...
    try:
        for i, path in enumerate(paths):
            observer.set_progress("phase7", 0.1 + 0.8 * (i / max(len(paths), 1)), note=path.stem)
            ...
            summaries.append(
                {
                    ...
                    "missing": man.get("missing_panels"),
                }
            )
            print(
                f"DONE {man['story_id']}: {man.get('status')} "
                f"panels={man.get('assembled_panels')} pdf={man.get('pdf_path')}"
            )

        out = OUTPUTS / "phase7" / "assemble_summary.json"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(json.dumps({"stories": summaries}, indent=2))
        catalog = rebuild_catalog()
        observer.complete(
            "phase7",
            note=f"assembled {len(summaries)} stories → {out}; library={catalog['count']}",
        )
        print(f"summary → {out}")
        print(f"library → {catalog['store_path']}")
    except Exception as e:  # noqa: BLE001
        observer.fail("phase7", str(e))
        raise
